...
#ifdef POPCNT64
// Hardware-accelerated population count (using POPCNT instruction)
static RES POPCNT_Hardware64(const BYTE * buf, SIZE_T len) {
  RES cnt = 0;

  auto ndwords = len / sizeof(DWORD64);
  for (; ndwords; ndwords--) {
...
#ifdef POPCNT64
// by Subbu N, http://www.strchr.com/crc32_popcnt?allcomments=1#comment_482
static RES POPCNT_HardwareSubbuN64(const BYTE * buf, SIZE_T len) {
  RES cnt = 0;

  auto align = sizeof(DWORD64) - (INT_PTR)buf & (sizeof(DWORD64) - 1);
  align = (len < align) ? 0 : align;
  if (align) {
    buf = (BYTE *)((INT_PTR)buf & (-(INT_PTR)sizeof(DWORD64)));
    auto first = *(DWORD64*)buf;
    first &= ~(0xFFFFFFFFFFFFFFFF >> (align * CHAR_BIT));
    cnt = __popcnt64(first);
    buf += sizeof(DWORD);
    len -= align;
  }
  ...
  */
  auto ndwords = len / sizeof(DWORD64);
  auto pInt = (DWORD64*)buf;
  while (ndwords)
    cnt += __popcnt64(pInt[--ndwords]); // performance optimization: Removed ptr addition.

  buf += len / sizeof(DWORD64) * sizeof(DWORD64);
...
static RES POPCNT_HardwareUnrolled(const BYTE * buf, SIZE_T len) {
  ...
  auto align = sizeof(DWORD) - (INT_PTR)buf & (sizeof(DWORD) - 1);
	align = (len < align) ? 0 : align;
	if (align) {
		buf = (BYTE *)( (INT_PTR)buf & (-(INT_PTR)sizeof(DWORD)) );
	  auto first = *(DWORD*)buf;
		first &= ~(0xFFFFFFFF >> (align * CHAR_BIT));
		cnt = __popcnt(first);
		buf += sizeof(DWORD);
		len -= align;
  ...
  auto ndqwords = len / (sizeof(DWORD) * 4);
  ...
  // This is faster, not entirely clear how

  auto p = (DWORD *)buf;;
	for(; ndqwords; ndqwords--)
    __popcnt(*p++) + __popcnt(*p++) + __popcnt(*p++) + __popcnt(*p++);
...
#ifdef POPCNT64
static RES POPCNT_HardwareUnrolled64(const BYTE * buf, SIZE_T len)
{
  //_mm_prefetch((CHAR *)buf, _MM_HINT_T0);
  RES cnt = 0;
  ...
  auto ndqwords = len / (sizeof(DWORD64) * 4);

  /* Original code was this:
  for (; ndqwords; ndqwords--) {
    cnt +=
      __popcnt64(*(DWORD64*)buf) +
      __popcnt64(*(DWORD64*)(buf + sizeof(DWORD64))) +
      __popcnt64(*(DWORD64*)(buf + sizeof(DWORD64) * 2)) +
      __popcnt64(*(DWORD64*)(buf + sizeof(DWORD64) * 3));
    buf += sizeof(DWORD) * 4;
  }  */
  ...
  auto p = (DWORD64 *)buf;;
  for (; ndqwords; ndqwords--)
    cnt += __popcnt64(*p++) + __popcnt64(*p++) + __popcnt64(*p++) + __popcnt64(*p++);

  buf = (BYTE *)p;

  len &= (sizeof(DWORD64) * 4 - 1);
  for (; len; len--)
    cnt += __popcnt(*buf++);
...
int main() {
  ...
  PrintHeader();
	...
	if (a[2] & POPCNT_SUPPORTED) {
		BenchmarkFunction(POPCNT_Hardware, "POPCNT-32", "Hardware");
    BenchmarkFunction(POPCNT_HardwareSubbuN, "POPCNT-32-SubbuN", "<a href=\"http://www.strchr.com/crc32_popcnt?allcomments=1#comment_482\">Hardware by Subbu N</a>");
    BenchmarkFunction(POPCNT_HardwareUnrolled,  "POPCNT-32-Unroll", "Hardware, unrolled");
...
#ifdef POPCNT64
    printf("\n");
    BenchmarkFunction(POPCNT_Hardware64, "POPCNT-64", "Hardware");
    BenchmarkFunction(POPCNT_HardwareSubbuN64, "POPCNT-64-SubbuN", "<a href=\"http://www.strchr.com/crc32_popcnt?allcomments=1#comment_482\">Hardware by Subbu N</a>");
    BenchmarkFunction(POPCNT_HardwareUnrolled64, "POPCNT-64-Unroll", "Hardware, unrolled");
#endif
	...
	if (a[2] & SSSE3_SUPPORTED)
		BenchmarkFunction(POPCNT_SSE_PSHUFB, "SSSE3_PSHUFB", "<a href=\"http://wm.ite.pl/articles/sse-popcount.html\">SSSE3</a>");
	else
